...
if($_POST)
{
    $to_Email   	= "contactosyner@prodigy.net.mx"; //Replace with recipient email address
    $subject        = 'Nuevo contacto desde nuestro sitio web'; //Subject line for emails
    ...
    {
        $output = json_encode(array('type'=>'error', 'text' => 'Los campos de entrada están vacíos!'));
        die($output);
    }
    ...
    {
        $output = json_encode(array('type'=>'error', 'text' => 'El nombre es muy corto o vacío!'));
        die($output);
    }
    ...
    */
    ...
    if(!$sentMail)
    {
        $output = json_encode(array('type'=>'error', 'text' => 'No se pudo enviar el correo! Por favor revisa tu configuración de correo PHP.'));
        die($output);
    }else{
